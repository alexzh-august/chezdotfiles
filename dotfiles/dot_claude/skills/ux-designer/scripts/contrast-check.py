...
def print_results(color1, color2, results):
    ...
    print("\nLevel AA:")
    print(
        f"  Normal text (< 18px):      {'✓ PASS' if results['aa_normal'] else '✗ FAIL'} (requires 4.5:1)"
    )
    print(
        f"  Large text (≥ 18px):       {'✓ PASS' if results['aa_large'] else '✗ FAIL'} (requires 3.0:1)"
    )
    print(
        f"  UI Components:             {'✓ PASS' if results['ui_components'] else '✗ FAIL'} (requires 3.0:1)"
    )

    # AAA Level
    print("\nLevel AAA:")
    print(
        f"  Normal text (< 18px):      {'✓ PASS' if results['aaa_normal'] else '✗ FAIL'} (requires 7.0:1)"
    )
    print(
        f"  Large text (≥ 18px):       {'✓ PASS' if results['aaa_large'] else '✗ FAIL'} (requires 4.5:1)"
    )
...
def main():
    ...
    try:
        ...
        print(
            "Please provide valid hex colors (e.g., #000000 or 000 or #fff)\n",
            file=sys.stderr,
        )
        sys.exit(2)
